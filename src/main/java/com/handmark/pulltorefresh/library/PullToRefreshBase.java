...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
	...
	public final boolean onInterceptTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_MOVE: {
			if (Math.abs(event.getY() - y) > DisplayUtil.dip2px(getContext(), 5)) {
				float temp = y;
				y = event.getY();
				if (y < temp) {// 上拉
					if(mMoveEvent != null)
	...
	protected LoadingLayoutProxy createLoadingLayoutProxy(final boolean includeStart, final boolean includeEnd) {
		...
		if (includeStart && mMode.showHeaderLoadingLayout()) {
			// mHeaderLayout.type = LoadingLayout.TYPE_HEADER;
			proxy.addLayout(mHeaderLayout);
		}
		if (includeEnd && mMode.showFooterLoadingLayout()) {
			// mFooterLayout.type = LoadingLayout.TYPE_FOOTER;
