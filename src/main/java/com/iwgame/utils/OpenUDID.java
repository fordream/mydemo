...
public class OpenUDID
{
	...
	private final static boolean _UseImeiFailback = false;// false if you don't
															// wanna include
															// READ_PHONE_STATE
															// permission
	// we recommend adding BT permission over READ_PHONE_STATE permission as the
	// will be less privacy concerns
	private final static boolean _UseBlueToothFailback = false; // false if you
																// don't wanna
																// include BT
																// permission or
																// android 1.6
	...
	private static String corpUDID;
	...
	/*
	 * public OpenUDID() { // TODO Auto-generated constructor stub }
	 */
	...
	{
		if (corpUDID == null)
			corpUDID = Md5(String.format("%s.%s", corpIdentifier, getOpenUDIDInContext()));
		LogUtil.d(TAG, "----->>diviceId="+corpUDID);
		return corpUDID;
	}
	...
	private static void generateSystemId()
	{
		...
		// only reach here for very narrow chances (android 2.2 , no wifi , no
		// bluetooth)
		// this always return sth.
		String fp = String.format("%s/%s/%s/%s:%s/%s/%s:%s/%s/%d-%s-%s-%s-%s", Build.BRAND, Build.PRODUCT, Build.DEVICE, Build.BOARD, Build.VERSION.RELEASE, Build.ID, Build.VERSION.INCREMENTAL, Build.TYPE, Build.TAGS, Build.TIME, Build.DISPLAY, Build.HOST, Build.MANUFACTURER, Build.MODEL);
