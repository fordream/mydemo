/**      
 * HttpUtil.java Create on 2013-6-7     
 *      
...
import com.iwgame.utils.object.FormFile;

/**
 * @ClassName: HttpUtil
 * @Description: TODO(...)
 * @author william
 * @date 2013-6-7 上午11:26:36
 * @Version 1.0
 * 
 */
public class HttpUtil {
	
	static public String TAG = "HttpUtils";
	 ...
	 * @throws IOException
	 */
	static public String sendRequestByHeaders(String url, String urlParam,
			Map headers) throws MalformedURLException, IOException {

		String destUrl = null;
		if (urlParam != null)
			destUrl = url + "?" + urlParam;
		else
			destUrl = url;

		HttpURLConnection http = (HttpURLConnection) new URL(destUrl)
				.openConnection();
		...
		byte[] responseByte = InputStreamToByte(in);
	 ...
	 * @throws IOException
	 */
	public static String postRequest(String url, String urlParam, Map headers)
		...
		if (headers != null && headers.keySet() != null) {
			...
			while (it.hasNext()) {
				String key = it.next();
				String value = (String) headers.get(key);
				http.addRequestProperty(key, URLEncoder.encode(value));
			}
		}
	...
	public static String put(String path, Map<String, String> params,
		...
		try {
			...
			fin = new BufferedInputStream(new FileInputStream(file.getFile()));
			int bufferSizeUpload = 1024;
			...
			respString = new String(readStream(conn.getInputStream()));
			return respString;
	...
	public static String get(String path, Map<String, String> params,
		...
		if (params != null && !params.isEmpty()) {
			...
			if (path.indexOf("?") > 0) {
				realpath = path + "&" + parambuilder.toString();
			} else {
				realpath = path + "?" + parambuilder.toString();
			}
		}
		LogUtil.d(TAG, "------->realpath:"+realpath);
	 ...
	 * @throws IOException
	 */
	static public byte[] sendRequest(String url, String urlParam, Map headers)
			throws MalformedURLException, IOException {
	 ...
	 * @throws Exception
	 */
	public static byte[] postRequest(String path, byte[] data) {
		try {
			URL url = new URL(path);
			HttpURLConnection conn = (HttpURLConnection) url.openConnection();
			conn.setRequestMethod("POST");
			...
			{
				System.err.println("*******HttpUtil.postRequest 请求返回错误：conn.getResponseCode() = " + conn.getResponseCode());
				System.err.println("*******请求内容 ：path = " + path+";data ="+String.valueOf(data));
			}
		}catch (Exception e) {
